...
{
    /// <summary>
    /// Class that represents medium strategy for CPU player
    /// </summary>
    public class StrategyMedium : Strategy
    {
        /// <summary>
        /// Default ctor for strategy medium
        /// </summary>
        ...
        protected override int EvaluateMove(Game g, GameMove move)
        {
            ...
            int opDistance = Math.Max(Math.Abs(op.Figures[0].I - move.BuildI), Math.Abs(op.Figures[0].J - move.BuildJ)) + Math.Max(Math.Abs(op.Figures[1].I - move.BuildI), Math.Abs(op.Figures[1].J - move.BuildJ));
